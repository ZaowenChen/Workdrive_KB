...
def upsert_document(row: Dict):
    with _conn() as conn:
        ...
        conn.execute(
            """
            INSERT INTO documents(file_id,name,path,size,created_time,modified_time,suffix,last_seen)
            VALUES (?,?,?,?,?,?,?,datetime('now'))
            ON CONFLICT(file_id) DO UPDATE SET
              ...
              suffix=excluded.suffix, last_seen=datetime('now');
            """,
            (
                row["file_id"],
                row["name"],
...
def upsert_labels(file_id: str, labels: Dict, source: str, confidence: float, needs_review: int):
    with _conn() as conn:
        conn.execute(
            """
            INSERT INTO labels(file_id,doc_type,model_type,subsystem,language,source,confidence,needs_review)
            VALUES (?,?,?,?,?,?,?,?)
            ON CONFLICT(file_id) DO UPDATE SET
...
def all_for_csv():
    with _conn() as conn:
        ...
        SELECT d.file_id, d.path, d.name, d.size, d.modified_time,
               l.doc_type, l.model_type, l.subsystem, l.language, l.source, l.needs_review
...
def update_from_csv_row(row: Dict):
    with _conn() as conn:
        conn.execute(
            """
            UPDATE labels SET doc_type=?, model_type=?, subsystem=?, language=?, source=?, needs_review=?
            WHERE file_id=?
            """,
            (
                row["doc_type"],
